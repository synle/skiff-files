...
describe("duplicateName", () => {
  it("appends -copy-YYYY-MM-DD-HH-MM to a file (preserves extension)", () => {
    expect(duplicateName("notes.md", { now: NOW })).toBe("notes-copy-2026-05-08-13-22.md");
  });
  ...
  it("strips a previous -copy-<timestamp> suffix before re-appending", () => {
    ...
    expect(duplicateName(once, { now: later })).toBe("notes-copy-2026-05-08-13-27.md");
  });
  ...
  it("strip-and-replace also works for folders", () => {
    ...
    expect(duplicateName(once, { isDir: true, now: later })).toBe(
      "My Project-copy-2026-05-08-14-00",
    );
  });

  it("uses 2-digit zero-padded month / day / hour / minute", () => {
    const earlyJan = new Date(2026, 0, 3, 5, 7);
    expect(duplicateName("a.txt", { now: earlyJan })).toBe("a-copy-2026-01-03-05-07.txt");
  });

  it("dotfiles (no real extension) keep their leading dot", () => {
    expect(duplicateName(".env", { now: NOW })).toBe(".env-copy-2026-05-08-13-22");
  });
  ...
  it("strips a -copy-<ts>-N collision suffix before re-appending", () => {
    ...
    expect(duplicateName("untitled-copy-2026-05-08-13-32-2.txt", { now: NOW })).toBe(
      "untitled-copy-2026-05-08-13-22.txt",
    );
  });
  ...
  it("strips multiple stacked -copy-<ts> suffixes", () => {
    ...
    expect(
      duplicateName("notes-copy-2026-05-08-13-22-2-copy-2026-05-08-13-32.md", { now: NOW }),
...
describe("uniqueDuplicateName", () => {
  it("returns the base name when no collision", () => {
    expect(uniqueDuplicateName("notes.md", new Set(), { now: NOW })).toBe(
