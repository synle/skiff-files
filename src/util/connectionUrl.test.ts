...
import { connectionId, connectionUrl, parseConnectionId } from "./connectionUrl";

describe("connectionId", () => {
  it("builds `user@host:port` for non-default port", () => {
    expect(connectionId({ kind: "sftp", host: "nas.local", port: 2222, user: "syle" })).toBe(
      "syle@nas.local:2222",
    );
  });

  it("elides default port per scheme (sftp=22)", () => {
    expect(connectionId({ kind: "sftp", host: "nas", port: 22, user: "syle" })).toBe("syle@nas");
  });

  it("elides default port per scheme (ftp=21)", () => {
    expect(connectionId({ kind: "ftp", host: "mirror", port: 21, user: "anonymous" })).toBe(
      "anonymous@mirror",
    );
  });

  it("elides default port per scheme (smb=445)", () => {
    expect(connectionId({ kind: "smb", host: "10.0.0.5", port: 445, user: "admin" })).toBe(
      "admin@10.0.0.5",
    );
  });

  it("defaults empty SMB user to `guest`", () => {
    expect(connectionId({ kind: "smb", host: "nas", port: 445, user: "" })).toBe("guest@nas");
  });

  it("defaults empty FTP user to `anonymous`", () => {
    expect(connectionId({ kind: "ftp", host: "mirror", port: 21, user: "" })).toBe(
      "anonymous@mirror",
    );
  });
});
...
describe("connectionUrl", () => {
  it("prefixes the scheme", () => {
    expect(connectionUrl({ kind: "smb", host: "nas", port: 445, user: "admin" })).toBe(
      "smb://admin@nas",
    );
  });

  it("keeps non-default port", () => {
    expect(connectionUrl({ kind: "smb", host: "nas", port: 1445, user: "admin" })).toBe(
      "smb://admin@nas:1445",
    );
  });
});
