...
describe("GalleryThumb", () => {
  ...
  it("does not call fs_thumbnail for non-image kinds", () => {
    ...
    expect(invoke).not.toHaveBeenCalledWith("fs_thumbnail", expect.anything());
  });

  it("does not call fs_thumbnail for remote paths", () => {
    render(<GalleryThumb path="sftp://host/a.jpg" kind="image" size={96} remote />);
    expect(invoke).not.toHaveBeenCalledWith("fs_thumbnail", expect.anything());
  });
  ...
  it("renders the kind icon as a fallback when the image isn't loaded yet", () => {
    ...
    const { container } = render(<GalleryThumb path="/a/photo.jpg" kind="image" size={96} />);
