...
- **Phase 4b (in progress)** — smart-batch conflict policies landed (overwriteOlder / replaceSmaller / replaceIfSizeDifferent / renameTarget / renameOlderTarget — TeraCopy parity for the per-file decisions that don't need a UI prompt). Still pending: cross-protocol src/dest, the interactive TeraCopy modal (Overwrite/Skip/Keep both — applies-to-all-remaining), pause/resume, `cpstamp`/`cprepo`/`dedup` modes, saved jobs.
- **Phase 5+** — see TODO.md
