...
- **Phase 4b (in progress)** — smart-batch conflict policies (overwriteOlder / replaceSmaller / replaceIfSizeDifferent / renameTarget / renameOlderTarget — TeraCopy parity for per-file decisions). `cpstamp` (timestamped single-file copy), `dedup` (md5+size duplicate scan → `_recycleBin/`), and `cprepo` (sync git ls-files only) all wired through Tauri commands + Transfers page sections. Still pending: cross-protocol src/dest, interactive TeraCopy modal (Overwrite/Skip/Keep both — applies-to-all-remaining), pause/resume, saved jobs.
- **Phase 5+** — see TODO.md
