...
- **No `process.env` / `std::env` reads at module load.** The variable _name_ alone is a phishing target. Read env inside runtime functions only; never echo.
