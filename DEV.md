...
│   │   ├── client.ts             # Backend-agnostic dispatch (`dispatchByLocation`)
...
- **New fs verbs MUST go through `dispatchByLocation` in `src/api/client.ts`.** Don't hand-roll a `if (kind === "sftp" || kind === "ftp" || kind === "smb")` per verb. The 0.2.270 SMB bug cluster — `mkdir` / `createEmptyFile` / `removeOrTrashMany` / `rename` all silently no-op'd or surfaced `CanonicalizePath { original: "smb://..." }` — was caused by every verb owning its own routing branches and one verb forgetting an arm. The 0.2.271 consolidation extracted the dispatch into one helper; each verb declares one `remote: (id, remotePath, kind) => …` and TypeScript surfaces the kind union so adding a new backend means widening one type. See ARCHITECTURE.md → "Routing model (0.2.271+)" for the full contract + an example. Regression coverage lives in `src/api/client.test.ts` (69 cases pinning every verb's per-backend route).
